	...
	MobileSelectArea.prototype = {
		...
		getData: function() {
			...
			if (typeof this.settings.data == "object") {
				this.data = this.settings.data['data'];
			} else {
				$.ajax({
					dataType: 'json',
					cache: true,
		...
		bindEvent: function() {
			...
			this.trigger.tap(function(e) {
				...
				for (var i = 0; i < _this.level; i++) {
					if(i == _this.level-1){
						dlgContent += '<div style="width:'+Math.floor(2/(_this.level+1)*100)+'%;"></div>';
					}else{
						dlgContent += '<div style="width:'+Math.floor(1/(_this.level+1)*100)+'%;"></div>';
					}
		...
		f: function(data) {
			...
			if (_this.index !== 0 && _this.value[_this.index - 1] == "0") {
				...
				for (var j = 0, len = item.length; j < len; j++) {
					...
					str += '<dd pid="' + pid + '" class="' + cls + '" ref="' + id + '">' + (item[j].region_name || item[j].region_message) + '</dd>';
				}
			}
			str += "</dl>";
			var newdom = $(str);
